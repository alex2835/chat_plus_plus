...
public:
    ChatClient( ClientDatabase& database ) : websocket_( ioContext_ ), database_( database )
    {
    ...
    void sendMessage( const std::string& message )
    {
        ...
        try
        {
            PostMessageRequest request{
                .user = userName_, .room = std::string(), .message = message };

            auto messageJson = json( makeMessage( ClientMessageType::PostMessage, request ) );

            websocket_.async_write( asio::buffer( messageJson.dump() ),
